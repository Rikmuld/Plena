...
namespace Energy {
    ...
    export function addEnergy(event: MouseEvent) {
        switch (curr) {
            case 0: solar.add([Mouse.getX(world), Mouse.getY(world)]); break
            case 1: city.add([Mouse.getX(world), Mouse.getY(world)]); break
            case 2: factory.add([Mouse.getX(world), Mouse.getY(world)]); break
        }
    }
...
function setup() {
    ...
    world.changeProjection(3840, 3840*(window.innerHeight / window.innerWidth))

    solar = new Energy(Textures.SOLAR, Color.Yellow.gold(1), 373, Energy.solarE)
    city = new Energy(Textures.CITY, Color.Blue.blueMidnight(1), 240, Energy.cityE)
    factory = new Energy(Textures.FACTORY, Color.Gray.graySlateDark(1), 415, Energy.factoryE)

    grix = new ShapeGrix().drawmodes([DrawModes.TRIANGLES]).quad(100, 100).populate()
    boxLine = new ShapeGrix().drawmodes([DrawModes.LINES_LOOP]).add([0, 0, 200, 0, 200, 50, 0, 50]).setColor(Color.White.white(1)).populate()

    machineGrix = Grix.fromSprite(Assets.loadSprite("machine.png", Assets.PIXEL_NORMAL)
        .addImg(Textures.FACTORY, 0, 0, 415, 298)
        .addImg(Textures.SOLAR, 420, 0, 373, 275)
        .addImg(Textures.CITY, 840, 0, 240, 230))
...
function render(delta: number) {
    ...
    machineGrix.moveTo(world.getWidth() - 20, 20)
    machineGrix.render()
